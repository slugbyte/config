...
class ContextView implements Component {
	...
	private rebuild(width: number): void {
		...
		if (!this.data.usage) {
			...
			const percentOfWindow = (tokens: number) => ((tokens / u.contextWindow) * 100).toFixed(1);
			const bar =
				renderUsageBar(
					this.theme,
					{
				...
				" " +
				dim("sys ") +
				this.theme.fg("accent", `${percentOfWindow(sysInMessages)}%`) +
				" " +
				dim("tools ") +
				this.theme.fg("warning", `${percentOfWindow(u.toolsTokens)}%`) +
				" " +
				dim("convo ") +
				this.theme.fg("success", `${percentOfWindow(convoInMessages)}%`) +
				" " +
				dim("free ") +
				this.theme.fg("dim", `${percentOfWindow(u.remainingTokens)}%`);
			lines.push(bar);
		}
